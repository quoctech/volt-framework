...
?><div>
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-2xl font-semibold text-slate-900"><?= $isEdit ? 'Edit Role' : 'New Role' ?></h1>
        <p class="mt-1 text-sm text-slate-500"><?= $isEdit ? 'Chỉnh sửa thông tin role.' : 'Tạo role mới để phân quyền cho người dùng.' ?></p>
    </div>
    ...
    <div class="border border-slate-300 bg-white p-6">
        <form action="<?= $isEdit ? site_url("desk/roles/update/{$role->name}") : site_url('desk/roles/store') ?>" method="post">
            ...
            <div class="grid gap-5">
                <div>
                    <label class="mb-1.5 block text-xs font-semibold uppercase tracking-[0.12em] text-slate-500" for="label">Label</label>
                    <input
                        id="label"
                        name="label"
                        type="text"
                ...
                <div>
                    <label class="mb-1.5 block text-xs font-semibold uppercase tracking-[0.12em] text-slate-500" for="name">Name</label>
                    <input
                        id="name"
                        name="name"
                        type="text"
                    ...
                    >
                    <?php if (! $isEdit): ?>
                        <p class="mt-1 text-xs text-slate-500">Chỉ chấp nhận chữ thường, số và underscore. Tự động sinh từ Label nếu để trống.</p>
                    <?php endif; ?>
                </div>

                <div>
                    <label class="mb-1.5 block text-xs font-semibold uppercase tracking-[0.12em] text-slate-500" for="description">Description</label>
                    <textarea
                        id="description"
                        name="description"
                        rows="3"
