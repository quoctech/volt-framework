...
/** @var array<int, array{fieldname:string,label:string,fieldtype:string}> $columns */
/** @var string $entityStudly */
/** @var string $listUrl */
/** @var string $moduleSnake */
/** @var string $entitySnake */
if ($isSubmittable) {
    $columns[] = ['fieldname' => 'workflow_state', 'label' => 'State', 'fieldtype' => 'Data'];
}
$deleteUrlBase = site_url("{$moduleSnake}/api/{$entitySnake}/delete");
...
<body class="min-h-screen bg-zinc-100 text-base text-zinc-900">
    <div x-data="VoltListApp({
            ...
            deleteUrlBase: <?= esc(json_encode($deleteUrlBase, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), 'attr') ?>,
            ...
            submitUrlBase: <?= esc(json_encode($submitUrlBase, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), 'attr') ?>,
            approveUrlBase: <?= esc(json_encode($approveUrlBase, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), 'attr') ?>,
            cancelUrlBase: <?= esc(json_encode($cancelUrlBase, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), 'attr') ?>,
            amendUrlBase: <?= esc(json_encode($amendUrlBase, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), 'attr') ?>
        ...
        <header class="mb-4 flex items-center justify-between border border-zinc-300 bg-white px-4 py-3">
            ...
            <div class="flex gap-2">
                ...
                <a href="<?= esc($createUrl) ?>" class="inline-flex items-center border border-slate-900 bg-slate-900 px-3 py-2 font-semibold text-white hover:bg-slate-800">Create <?= esc($entityStudly) ?></a>
            </div>
        </header>
    ...
    <script src="<?= base_url('assets/js/volt/entity_list.js') ?>"></script>
</body>
</html>
