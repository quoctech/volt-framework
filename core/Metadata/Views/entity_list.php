...
?><div class="space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
            ...
            <div class="mt-1 text-sm text-gray-600">Danh sách các entity trong hệ thống. Click vào tên entity để xem dữ liệu.</div>
        </div>
        ...
        <form method="get" action="<?= site_url('desk/entities') ?>" class="flex gap-2">
            ...
            <button type="submit" class="rounded border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-1">Filter</button>
        </form>
    </div>
        ...
        <table class="min-w-full text-sm">
            <thead>
                <tr class="border-b border-gray-300 bg-gray-100">
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Entity</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Label</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Module</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Autoname</th>
                    <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Submittable</th>
                </tr>
            </thead>
            <tbody>
                <?php if ($entities === []): ?>
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-gray-500">Không có entity nào khớp bộ lọc hiện tại.</td>
                    </tr>
                <?php endif; ?>
                ...
                <?php foreach ($entities as $i => $entity): ?>
                    ...
                    <tr class="border-b border-gray-200 <?= $i % 2 === 0 ? 'bg-white' : 'bg-gray-50' ?>">
                        ...
                        <td class="px-4 py-3"><?= ! empty($entity['is_submittable']) ? 'Yes' : 'No' ?></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
