...
<body class="min-h-screen bg-zinc-100 text-base text-zinc-900">
    <div x-data="employee_skill_mapListApp({
            ...
            deleteUrlBase: <?= esc(json_encode(site_url('hrms/api/employee_skill_map/delete'), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), 'attr') ?>,
        ...
        <header class="mb-4 flex items-center justify-between border border-zinc-300 bg-white px-4 py-3">
            <div>
                ...
                <p class="text-zinc-500">Generated list route: <?= esc('/hrms/employee_skill_map') ?></p>
            </div>
            <div class="flex gap-2">
                <a href="<?= esc($builderUrl) ?>" class="border border-zinc-300 px-3 py-2 hover:bg-zinc-50">Open Builder</a>
                <a href="<?= esc($createUrl) ?>" class="inline-flex items-center border border-slate-900 bg-slate-900 px-3 py-2 font-semibold text-white hover:bg-slate-800">Create EmployeeSkillMap</a>
            </div>
        </header>
    ...
    <script><?php readfile(APPPATH . 'Modules/Hrms/Entities/EmployeeSkillMap/employee_skill_map_list.js'); ?></script>
</body>
</html>